...
def nbMonths(oldCar, newCar, savings, percent):
        months = 0
        budget = oldCar
        while newCar > budget:
            months += 1
            if months % 2 == 0:
                percent += 0.5
            oldCar -= (oldCar * (percent/100))
            newCar -= (newCar * (percent/100))
            budget = oldCar + (savings * months)
        return [months, round(budget - newCar)]
